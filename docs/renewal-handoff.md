...
2026-09-19 に、それまでの16枚が**既存の性格診断サイトのキャラクター画像を画像生成 AI に読み込ませて変換したもの**だと分かった（運営者の申告）。元の画像に依拠しているため、配布ページの「画像の権利は運営者にあります」も、免責の「オリジナルの画像です」も成り立たない。

同じ日に、次の順でサイトから外した。

1. 配布ページの `PAUSED` を true にして、第三者への再配布を止めた
2. 画面（`CharacterFigure`）と生成画像（`lib/og/type-art.tsx`）の参照を、型コードを薄く置く地紋に替えた
3. `public/characters/{TYPE}.png` の16枚を削除した（git の履歴には残るが、サイトからは配信されない）
4. 免責と台帳の記述を、画像がない状態に合わせた

いまサイトに出ている絵は1枚もない。**残っているのは、描き下ろした16枚を入れ直すこと**。プロンプトは `docs/character-prompts.md` にある。参照画像を上げず、あのテキストだけで作る。できあがったら `public/characters/` に置いて `npm run build` を回せば、画面も生成画像も自動で戻る（`CharacterFigure` と `type-art.tsx` に画像を戻す作業は別途必要）。生成ツール・時期・プロンプトを `docs/asset-credits.md` 1-2 に記録し、`PAUSED` を false に戻すと配布も再開できる。

法的な線引きが要るなら、専門家に一度見てもらうのが確実。
