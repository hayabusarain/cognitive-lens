...
**2026-09-19 の追記**：現行の16枚は、既存の性格診断サイトのキャラクター画像を画像生成 AI に読み込ませて
変換したものだと分かった（運営者の申告。`asset-credits.md` 1-1）。同日、サイトの全ページと生成画像から外し、
ファイルも削除した。いまは `CharacterFigure` と `lib/og/type-art.tsx` が、窓に型コードを薄く置いている。
描き下ろすときのプロンプトは `docs/character-prompts.md` にある。参照画像は使わない。
