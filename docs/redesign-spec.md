...
| `/sitemap.xml`、`/robots.txt`、`/ads.txt` | クローラー向け | 静的 | 維持・内容更新 |
...
1〜3 をファーストビューとし、広告を置かない（Q15）。共有を3番目に置くのも同じ理由で、診断を終えた直後に画像を保存できるようにするためだ。広告枠は4以降の節の間に置く。
...
| `share` | X への投稿ボタンを押したとき | `method`（`x`）、`content_type`（`result` か `bingo`）、`item_id`（型コード） |
| `save_image` | 保存の流れ（4-3）で共有シートかダウンロードが成功したとき、または長押しの案内を表示したとき | `method`（`share_sheet`、`download`、`long_press`）、`content_type`、`item_id` |

長押しの案内では、実際に保存したかは取得できない。案内を表示した時点で送る。回答内容と各軸の割合は送らない。

現行の `CookieConsentBanner`（利用を続けると同意とみなす方式）は外し、同意ボタンは置かない。GA4 を使うことと送る情報は、プライバシーポリシーで公表する（Q16、日本の外部送信規律への対応）。GA4 の配信とポリシーの公表は同じステップで公開する（ステップ 3-19）。

欧州経済領域と英国からのアクセスには、AdSense 管理画面の「プライバシーとメッセージ」（Google 認定の同意管理ツール）で同意画面を出す（N8）。この画面は日本からのアクセスには出ないので、日本向けに同意ボタンを置かない方針と矛盾しない。GA4 はこの同意ツールの対象外で、プライバシーポリシーでの公表のままにする。
...
ここからフェーズ3の 3-24 までは作業用ブランチで進め、本番には出さない。
...
| 3-2 | `/ja/result/INTJ` だけを生成する | 1ページ | ビルド出力に静的生成として出る。HTML に `hgroup` の h1（INTJ）と副題（スフィンクス）、本文、canonical がある。3-4 節の1〜3 の範囲に広告がない。運営者が分量を見て、行動プロトコルを載せるか決める（Q9 の保留） | — |
...
| N8：AdSense の「プライバシーとメッセージ」で、欧州経済領域と英国向けの同意画面を有効にする | 3-23 |
| Q15：AdSense の広告枠 ID | 3-23 |
...
| N6 | 各ステップを1つずつ本番へ出すか、ブランチでまとめるか | フェーズ0は即本番、フェーズ1はステップごとに本番、フェーズ2〜3はブランチでまとめてプレビュー確認後に一括公開、フェーズ4はステップごと | 8章の冒頭、ステップ 3-24 |
| N7 | seed-protocols をフェーズ0で先に削除するか | すぐ削除し、0-1 の push と同じコミットに入れる。認証なしで書き込みが起きる API をほかにも調べる | ステップ 0-1（完了）、フェーズ0の調査結果 |
| N8 | 同意ボタンを置かない方針で、欧州からのアクセスへの AdSense の広告をどう扱うか | AdSense 管理画面の「プライバシーとメッセージ」（Google 認定の同意管理ツール、無料）を使う。欧州経済領域と英国からのアクセスにだけ同意画面が出て、日本からのアクセスには何も出ない。Q16 の「同意ボタンを置かない」は日本向けの話なので矛盾しない。GA4 はこの同意ツールとは別で、プライバシーポリシーでの公表のまま | 3-7、ステップ 3-23、9-2 |
