...
16タイプの結果を、タイプ色の太い枠のトレーディングカードのように見せる。スクリーンショットで SNS に載せたくなる強さはカードと型コードで出し、本文は暗い面の上で読みやすく保つ。部品の実物は `/styleguide`（ステップ 3-22 で削除）で見られる。
...
- h1 は1ページに1つ。結果ページは `<hgroup>` に h1（型コード、`font-display text-hero`）と `<p>`（呼称）を入れる（仕様書 3-4）。組み方は `/styleguide` の「タイプの枠」にある
