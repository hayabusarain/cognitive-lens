...
import { getTargetQuestions } from "@/lib/data-provider";
...
export default function TargetDiagnosisPage() {
  ...
  const TARGET_QUESTIONS = getTargetQuestions(lang);
  ...
  const handleAnswer = (value: string) => {
    ...
    if (currentStep < 19) {
      ...
      // 計算中UIを少し見せてから結果ページへ
      setTimeout(() => {
        router.push(`/${lang}/result?type=${finalType}`);
      }, 2000);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-slate-950 relative flex flex-col font-sans overflow-hidden text-slate-100">
      ...
      <header className="relative z-10 w-full p-4 flex items-center justify-between">
        <Link href={`/${lang}`} className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors">
          <ArrowLeft size={16} />
        ...
        {isCalculating ? (
          <div className="flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
            ...
            <h2 className="text-2xl font-bold text-white mb-2 tracking-wide">
              {lang === "en" ? "Analyzing..." : "解析中..."}
            </h2>
            <p className="text-slate-500 font-medium text-sm leading-relaxed mt-4 animate-pulse text-center">
              {lang === "en" 
                ? <>Extracting 16 type profile<br/>from target's behavior patterns...</> 
