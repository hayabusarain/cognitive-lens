...
export default function SkipPathPage({ params }: { params: Promise<{ lang: string }> }) {
      ...
      <div className="flex-1 max-w-xl mx-auto w-full px-5 py-10 flex flex-col gap-8">
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 mb-6 px-4 py-1.5 rounded-full text-xs font-medium badge-dark">
            <Zap size={12}  />
            <span >
              {lang === "en" ? "SHORTCUT SCAN — Deep Analysis w/o Questions" : "SHORTCUT SCAN — 回答ログなし精密解析"}
            </span>
          </div>
          <h1
          ...
          >
            {lang === "en" ? "For those who already " : "自分のタイプを"}
            <span
              style={{
            ...
            >
              {lang === "en" ? "know their type." : "知っている人へ。"}
            </span>
          </h1>
          <p className="text-sm leading-relaxed" >
            {lang === "en" ? (
              <>Skip the 20 questions. Just tell us your type and your "repeating failure pattern".<br/>We'll completely dissect why your cognitive functions go out of control.</>
            ) : (
              <>20問の診断をスキップ。タイプと「繰り返す失敗パターン」だけで、<br />あなたの認知機能がなぜ暴走するのかを徹底解剖します。</>
            )}
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold" >
              {lang === "en" ? "Select your 16 Type" : "あなたの16タイプを選択"}
            </h2>
          </div>
          ...
          {selectedType && (
            <p className="text-xs text-center" >
              {lang === "en" ? `✓ Selected ${selectedType}` : `✓ ${selectedType}を選択中`}
            </p>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold" >
              {lang === "en" ? "Enter Data for Deep Scan" : "精密スキャン用データを入力"}
            </h2>
          </div>
          ...
          <div className="glass-card rounded-2xl p-5 space-y-3">
            <label className="block">
              <span className="text-xs font-bold tracking-wide" >
                {lang === "en" ? "What is a 'failure pattern' you keep repeating in your life?" : "あなたが人生で何度も繰り返してしまう「失敗のパターン」を教えてください"}
              </span>
              <span className="block text-[10px] mt-0.5" >
                {lang === "en" ? "We will analyze this as a cognitive function collapse. Be honest for better accuracy." : "認知機能の暴走として解析します。正直に書くほど精度が上がります。"}
              </span>
            </label>
            <textarea
            ...
            <div className="flex justify-between text-[10px]" >
              <span>
                {failurePattern.length < 5
                  ? (lang === "en" ? `Please enter at least ${5 - failurePattern.length} more characters` : `あと${5 - failurePattern.length}文字以上入力してください`)
                  : (lang === "en" ? "✓ Ready" : "✓ 入力完了")}
              </span>
        ...
        >
          <Sparkles size={15} />
          {lang === "en" ? "Analyze Deep Psychology" : "深層心理を解剖する"}
        ...
        {isStreaming && sections.length === 0 && (
          ...
          >
            ...
            <p className="text-xs" >
              {lang === "en" ? "Analyzing cognitive runaway patterns..." : "認知機能の暴走パターンを解析中..."}
            </p>
          </div>
        )}
        ...
        {rateLimited && (
          ...
          >
            ...
            <p className="font-bold text-sm" >{lang === "en" ? "Analysis Engine is Busy" : "解析エンジンが混雑中"}</p>
            <p className="text-xs leading-relaxed" >
              {lang === "en" ? "You can scan again in 10 minutes. Please wait a moment and try again." : "10分後に再スキャン可能です。しばらく経ってから再度お試しください。"}
            </p>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {isGenerated && (
              <div className="space-y-3 pt-2">
                <button
                  onClick={() => router.push(`/${lang}/result?type=${selectedType}`)}
                ...
                >
                  <Sparkles size={14} />
                  {lang === "en" ? `View Full Profile for ${selectedType}` : `${selectedType} の完全プロファイルを見る`}
                ...
                >
                  {lang === "en" ? "Re-analyze with a different pattern" : "別パターンで再解析する"}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="text-center text-xs space-y-1 pt-2" >
          <p>{lang === "en" ? "Want to take the full test?" : "診断から始める場合は"}</p>
          <Link href={`/${lang}/test`} className="underline underline-offset-2" >
            {lang === "en" ? "Go to 20-Question Test →" : "20問の診断へ →"}
          </Link>
        </div>
      </div>
    </main>
